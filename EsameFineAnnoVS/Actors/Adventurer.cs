...
{
    internal class Adventurer : Actor
    {

        public Adventurer(string textureName = "AdventurerIdleD") : base(textureName)
        {
            IsActive = true;
                                                              ...
                                                              //(if the direction was not the same)
        {
            ...
            if (direction != Vector2.Zero && same == false)
            {
                ...
                if (direction.Y == 1)
                {
                    texture = GfxMngr.GetTexture("AdventurerWalkD-0-NBG");
                    sprite = new Sprite(texture.Width, texture.Height/4);
                }
                else if (direction.Y == -1)
                {
                    texture = GfxMngr.GetTexture("AdventurerWalkU-NBG");
                    sprite = new Sprite(texture.Width, texture.Height/4);
                }
                else if (direction.X == 1)
                {
                    texture = GfxMngr.GetTexture("AdventurerWalkR-NBG");
                    sprite = new Sprite(texture.Width, texture.Height/4);
                }
                else
                {
