...
{
    internal class Doggo : Actor
    {


        public Doggo(string textureName = "DogIdleD") : base(textureName)
        {
                                                             ...
                                                             //(if the direction was not the same)
        {
            ...
            if (direction != Vector2.Zero && same == false)
            {
                ...
                if (direction.Y == 1)
                {
                    texture = GfxMngr.GetTexture("DoggoWalkD-NBG");
                    sprite = new Sprite(texture.Width, texture.Height / 4);
                }
                else if (direction.Y == -1)
                {
                    texture = GfxMngr.GetTexture("DoggoWalkU-NBG");
                    sprite = new Sprite(texture.Width, texture.Height / 4);
                }
                else if (direction.X == 1)
                {
                    texture = GfxMngr.GetTexture("DoggoWalkR-NBG");
                    sprite = new Sprite(texture.Width, texture.Height / 4);
                }
                else
                {
